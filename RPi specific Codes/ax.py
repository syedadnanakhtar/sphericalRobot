...
rx = GPIO.LOW			# Low for TX mode
tx = GPIO.HIGH			# High for RX mode
...
def sync_write(indexes, reg, values):
	...
	port.write((chr(0xFF)+chr(0xFF)+chr(BROADCASTID)+chr(length)+chr(AX_SYNC_WRITE)+chr(reg) + chr(len(values[0]))))
